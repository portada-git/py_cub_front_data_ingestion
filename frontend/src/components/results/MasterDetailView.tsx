...
export interface MasterDetailProps {
  ...
  onDetailLoad?: (masterRow: Record<string, unknown>) => Promise<Record<string, unknown>>;
  masterTitle?: string;
  detailTitle?: string;
...
const MasterDetailView: React.FC<MasterDetailProps> = ({
  ...
  const [detailData, setDetailData] = useState<Record<string, Record<string, unknown>>>({});
  ...
  const toggleExpanded = async (row: Record<string, unknown>, index: number) => {
  ...
  const loadDetailData = async (row: Record<string, unknown>, rowKey: string) => {
      ...
      render: (_, row: Record<string, unknown>, index: number) => {
        ...
        return (
          <button
            ...
            title={expandMode === 'inline' ? (isExpanded ? 'Contraer' : 'Expandir') : 'Ver detalles'}
          >
            {isLoading ? (
  ...
  const renderDetailContent = (row: Record<string, unknown>, rowKey?: string) => {
                  ...
                  return (
                    <React.Fragment key={rowKey}>
                      ...
                      {expandMode === 'inline' && isExpanded && (
                        <tr>
                          <td colSpan={enhancedMasterColumns.length} className="px-6 py-4 bg-gray-50">
                ...
                <div className="flex flex-col h-full bg-white shadow-xl">
                  <div className="px-4 py-6 bg-gray-50 sm:px-6">
                    <div className="flex items-center justify-between">
                      <h2 className="text-lg font-medium text-gray-900">{detailTitle}</h2>
                      <button
