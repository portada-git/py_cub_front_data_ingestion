...
const UploadStatusTracker: React.FC<UploadStatusTrackerProps> = ({
    ...
    try {
      ...
      const newStatusData: UploadStatusData = {
        status: response.status === 'completed' ? 'completed' : 
                ...
                response.status === 'processing' ? 'processing' : 'uploading',
      ...
      if (response.status === 'failed') {
        newStatusData.error = response.message || 'Error desconocido durante el procesamiento';
        if (onError) {
          onError(newStatusData.error);
        }
      }
  ...
  return (
    <UploadStatus
      ...
      onCancel={statusData.status === 'uploading' || statusData.status === 'processing' ? handleCancel : undefined}
      onRetry={statusData.status === 'error' ? handleRetry : undefined}
      onDismiss={statusData.status === 'completed' || statusData.status === 'error' ? handleDismiss : undefined}
      className={className}
    />
  );
};
